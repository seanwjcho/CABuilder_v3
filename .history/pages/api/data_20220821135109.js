                ...
                if (counter) { // prompt
                    if (lines[line].charAt(0) == "_") { //tokenized prompt
                        try {
                            ...
                            query = 'INSERT INTO prompts(prompt_text, program_id, tokenized) VALUES (?, ?, 1)';
                            ...
                            const words = tokenizedprompt.matchAll(/\$.*?\$/g);
                            for (const m of words) {
                                const token = m[0].slice(1, -1);
                                query = 'INSERT INTO tokens(token_text, prompt_id) VALUES (?, ?)';
                        ...
                        console.log("here is prompt_Text") ;
                        console.log(lines[line]);
                        try { 
                    ...
                    if (tokn) { //tokenized response
                        ...
                        const id = lines[line].match(/\*.*?\*/);
                        try { 
                            console.log("id here")
                            console.log(id[0].substring(1,-1));
                            query = 'UPDATE prompts SET primitive = ? WHERE prompt_id = ?';
                            values = [id, currentpromptid];
                            console.log("prompt" + lines[line]);
                            console.log("prompt text inserted");
                            console.log(currentpromptid);
                            console.log("prompt id obtained");
