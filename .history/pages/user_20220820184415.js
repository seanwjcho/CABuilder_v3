...
function App() {
  ...
  function getButton() {
    return <button className = "button2" type = "submit">Submit</button>;
  }
  ...
  setSubmitButton(["hello", "Submit input parameters"]);
  submitbutton.set("hello", "Submit input parameters");
      ...
      <div className="grid">
            ...
            <a className="card">
              ...
              <form onSubmit={getTokenizedResponse}>
                <div class = "rows">
                {tokenlist?.map((token) => <input type="text" key={token.token_text} name={token.token_text} placeholder={token.token_text}/>)}
                {submitbutton?.map((b) => <button key = {b.key} className = "button2" type = "submit">{b.value}</button>)}
                </div>
