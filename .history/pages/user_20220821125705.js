...
function App() {
  ...
  async function getTokenizedResponse() {
    ...
    for (const val of tokenlist.values()) {
      console.log(val.token_text)
      console.log(document.getElementById(val.token_text).value);
      tokenmap.set(val.token_text, document.getElementById(val.token_text));
    }
    tokenmap.set("id", tokenizedpromptid);

    const response = await fetch('/api/gettokenizedresponse', {
      method: 'POST',
      body: tokenmap
    }).then(response => response.json())
    .catch(err => console.log(err))
