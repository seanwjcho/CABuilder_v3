...
function App() {
  ...
  async function getTokenizedResponse() {
    ...
    console.log("tokenlisthere")
    console.log(tokenlist)
    for (const val of tokenlist.values()) {
      tokenmap.set(val.token_text, document.getElementById(val.token_text).value);
    }
    tokenmap.set("id", tokenizedpromptid);
    let jsonText = JSON.stringify(Array.from(tokenmap.entries()));
    ...
    .catch(err => console.log(err))
    console.log(response)
    console.log(response.response[0].response_text);
    setResponse(response.response[0].response_text);
      ...
      <div className="grid">
            ...
            <a className="card">
              ...
              <form>
                ...
                <button onClick = {getTokenizedResponse} className = "button2" type = "button">Submit input parameters</button>
                </div>
              </form>
